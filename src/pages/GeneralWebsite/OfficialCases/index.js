...
export default function OfficialCases() {
  ...
  useEffect(() => {
    try {
      api.get('api/v1/cases/state').then((response) => {
  ...
  function handleCityChoice(choice) {
    if (choice !== null) {
      ...
      const parsedCity = choice.label
        .toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '');
      console.log(parsedCity);
      api.get(`api/v1/cases/state?cidade=${parsedCity}`).then((response) => {
        console.log(response);
        if (response !== null) {
  ...
  return (
    <div className="external-container">
      ...
      <div className="official-cases-container row">
        <h1>
          Casos da COVID-19 em <strong>Pernambuco</strong> .
        </h1>
        ...
        <h1>
          Casos por município de <strong>Pernambuco</strong> .
        </h1>
        ...
        <h6>
          {' '}
          {state.updatedAt
            ? `Fonte: IRRD-PE. Atualizado em: ${moment(state.updatedAt).format('DD/MM/YYYY')}`
            : 'Fonte: IRRD-PE.'}
        </h6>
      </div>
      <Footer />
