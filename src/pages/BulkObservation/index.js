...
export default function BulkObservation() {
  ...
  useEffect(() => {
    ...
    fetch(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/26/municipios`)
      ...
      .catch((error) => {
        setSnack({ type: 'error', message: ' Verifique sua conexão e recarregue a página.' });
        setOpenSnack(true);
      });
  ...
  function isRequiredFilled() {
    if (neighborhood_name !== '' && city !== '' && case_type !== '' && number_of_cases !== 0 && info_source !== '') {
      return true;
    }
  ...
  async function postObserverReport(data) {
    try {
      ...
      //console.log(data);
      setSnack({ type: 'success', message: 'Caso cadastrado com sucesso' });
      setOpenSnack(true);
